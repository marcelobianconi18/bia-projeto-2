...
import { Layers, Clock, Search, Navigation, Download, Copy, Facebook, X, Target, Loader2, Check, ChevronLeft, Map as MapIcon, Box, Flame, MapPin, ChevronRight, Eye, SlidersHorizontal, Activity, ShieldCheck } from 'lucide-react';
...
export const DashboardUI: React.FC<DashboardUIProps> = ({ 
  ...
  const copyToClipboard = () => {
    const text = `
⚡ ESTRATÉGIA TÁTICA BIA - ${briefing.geography.city.toUpperCase()}
-----------------------------------------
🎯 Foco: ${briefing.marketPositioning}
📍 Raio de Ação: ${(settings.radius / 1000).toFixed(1)}km
💰 Custo Estimado: R$ 120,00/dia
        ...
        <div className="flex items-center gap-4 pointer-events-auto">
          ...
          <div className="bg-black/90 px-4 py-2.5 rounded border border-[#39ff14]/50 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] text-[#39ff14] font-bold uppercase tracking-widest">{briefing.geography.city} // SCAN_MODE</p>
            </div>
          </div>
        </div>
      ...
      <div className="absolute top-24 left-6 w-72 z-[1000] flex flex-col gap-4 pointer-events-none font-mono">
          ...
          <div className="space-y-6">
            <div>
               <div className="flex justify-between items-center mb-3">
                  <span className="text-[10px] text-slate-400 uppercase font-bold">Score Minimo</span>
            ...
            <div className="pt-4 border-t border-white/5 space-y-2">
               ...
               {[
                 ...
                 { id: 'showLogistics', label: 'RAIO_TATICO' },
                 { id: 'showCompetitors', label: 'RADAR_CONC' },
                 { id: 'hideNoise', label: 'FILTRO_ANTI_GHOST' }
               ].map(l => (
                 <button key={l.id} onClick={() => toggleSetting(l.id as any)} className={`w-full flex items-center justify-between p-3 border transition-all ${settings[l.id as keyof MapSettings] ? 'bg-[#39ff14]/5 border-[#39ff14]/40 text-[#39ff14]' : 'bg-transparent border-white/5 text-slate-600'}`}>
                   <span className="text-[10px] font-bold">{l.label}</span>
                   <div className={`w-1.5 h-1.5 ${settings[l.id as keyof MapSettings] ? 'bg-[#39ff14] shadow-[0_0_5px_#39ff14]' : 'bg-slate-800'}`} />
                 </button>
               ))}
            </div>
          </div>
        ...
        <div className="bg-black/90 rounded border border-[#39ff14]/30 p-5 pointer-events-auto shadow-2xl">
           <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2">
            <Activity size={14} /> MÉTRIQUEIS_BIA
          </h3>
          <div className="grid grid-cols-2 gap-4">
             <div className="bg-slate-900/50 p-3 border border-white/5">
                <span className="text-[8px] text-slate-500 block uppercase">Pop. Total</span>
                <span className="text-sm font-black text-[#39ff14]">{metrics.totalPopulation}</span>
             </div>
             <div className="bg-slate-900/50 p-3 border border-white/5">
                <span className="text-[8px] text-slate-500 block uppercase">Media Score</span>
                <span className="text-sm font-black text-[#39ff14]">{metrics.avgScore}%</span>
             </div>
          </div>
        </div>
      </div>
      ...
      <div className="absolute top-24 right-6 w-80 bottom-24 z-[1000] flex flex-col gap-4 pointer-events-none font-mono">
        <div className="bg-black/90 rounded border border-[#39ff14]/30 p-5 pointer-events-auto shadow-2xl shrink-0">
          <div className="flex justify-between items-end mb-4">
            ...
            <div className="bg-black/90 rounded border border-[#39ff14]/30 p-5 pointer-events-auto shadow-2xl">
                {metrics.avgScore > 75 ? "ALVO_CRITICO" : "TERRITORIO_NEUTRO"}
              </span>
            </div>
          ...
          <div className="w-full h-1 bg-slate-900 border border-white/5 overflow-hidden">
            <div className="h-full bg-[#39ff14] shadow-[0_0_10px_#39ff14] transition-all duration-1000" style={{ width: `${metrics.avgScore}%` }} />
          </div>
          <div className="mt-4 bg-slate-950 p-3 border border-[#39ff14]/10">
             <p className="text-[10px] text-[#39ff14]/80 italic leading-relaxed uppercase">>> "{analysis?.verdict}"</p>
          </div>
        </div>
        ...
        <div className="bg-black/90 rounded border border-white/10 pointer-events-auto flex flex-col flex-1 overflow-hidden shadow-2xl">
          <div className="p-4 border-b border-white/5 bg-slate-900/40 flex items-center justify-between">
            ...
            <span className="bg-[#bc13fe]/20 text-[#bc13fe] text-[8px] px-1.5 py-0.5 rounded font-black border border-[#bc13fe]/30">LIVE_SCAN</span>
          </div>
          ...
          <div className="p-4 bg-black border-t border-white/5 space-y-2">
             ...
             <button onClick={() => setShowExport(true)} className="w-full py-3 bg-[#39ff14] text-black rounded font-black text-[10px] tracking-widest flex items-center justify-center gap-2 transition-transform active:scale-95 shadow-lg shadow-[#39ff14]/20">
                <Navigation size={14} fill="black" /> EXPORTAR_ESTRATEGIA
             </button>
          </div>
        </div>
        {/* Explainability - compact card */}
        <ExplainabilityCard analysis={analysis} />
      </div>
            ...
            <div className="bg-black w-full max-w-2xl rounded border border-[#39ff14] p-10 relative">
               ...
               {exportStep === 'selection' && (
                  <>
                    ...
                    <p className="text-[#39ff14]/60 mb-10 font-bold text-[10px] uppercase">Selecione o protocolo de saida BIA_PROTOCOL_X</p>
               ...
               {exportStep === 'loading' && (
                   <div className="flex flex-col items-center justify-center py-12">
                       ...
                       <h3 className="text-xl font-bold text-white uppercase tracking-widest">Processando_Protocolo...</h3>
                   </div>
               )}
               {exportStep === 'result' && (
                   <div className="text-center">
                       <h2 className="text-2xl font-black text-white mb-6 flex items-center justify-center gap-2 uppercase"><Check className="text-[#39ff14]"/> Estratégia_Pronta</h2>
                       <button onClick={copyToClipboard} className={`w-full py-4 rounded font-black text-black transition-all flex items-center justify-center gap-2 ${copied ? 'bg-green-500' : 'bg-[#39ff14] shadow-[0_0_20px_#39ff14]/30 hover:scale-[1.02]'}`}>
                            {copied ? "COPIADO_PARA_ÁREA_DE_TRANSFERÊNCIA" : "COPIAR_ESTRATÉGIA_FINAL"}
                       </button>
                   </div>
               )}
            </div>
