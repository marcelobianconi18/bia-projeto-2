...
export const HotspotsMapPanel: React.FC<HotspotsMapPanelProps> = ({ map, onNavigate }) => {
  ...
  return (
    <div 
                <p className={`text-xs font-bold transition-colors ${spot.rank <= 3 ? 'text-white' : 'text-slate-300'} group-hover:text-white`}>{spot.name} <span className="ml-2">{spot.provenance && <ProvenanceBadge provenance={spot.provenance} />}</span></p>
        ...
        {hotspots.map((spot) => (
          ...
          >
            <div className="flex items-center gap-4">
              <span className={`text-[10px] font-black w-7 h-7 flex items-center justify-center rounded-lg transition-all ${
                spot.rank === 1 ? 'bg-yellow-500 text-slate-950 shadow-[0_0_15px_rgba(234,179,8,0.4)]' : 
              ...
              <div>
                <p className={`text-xs font-bold transition-colors ${spot.rank <= 3 ? 'text-white' : 'text-slate-300'} group-hover:text-white`}>
                  {spot.name}
                </p>
